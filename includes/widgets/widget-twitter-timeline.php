...
if(!class_exists('Epic_Twitter_Timeline_Widget')){
    class Epic_Twitter_Timeline_Widget extends WP_Widget {
        ...
        public function widget( $args, $instance ) {
            ...
            <a class="twitter-timeline" data-lang="es" data-theme="<?php echo esc_attr($instance['twitter_theme']);?>" data-link-color="<?php echo esc_attr($instance['twitter_link_color']);?>" href="<?php echo esc_url($instance['twitter_url']);?>">Tweets por <?php echo esc_html($instance['twitter_account']);?></a> <script async src="//platform.twitter.com/widgets.js" charset="utf-8"></script>
            <?php
        ...
        public function form( $instance ) {
            $twitter_account = isset($instance['twitter_account']) ? sanitize_text_field($instance['twitter_account']) : __('todoapuestas', 'epic');
            $twitter_url = isset($instance['twitter_url']) ? esc_url($instance['twitter_url']) : esc_url(get_theme_mod('epic_social_twitter'));
            $twitter_theme = isset($instance['twitter_theme']) ? sanitize_text_field($instance['twitter_theme']) : 'light';
            $twitter_link_color = isset($instance['twitter_link_color']) ? sanitize_hex_color($instance['twitter_link_color']) : '#111111';
            ?>
            <p>
                <label for="<?php echo $this->get_field_id( 'twitter_account' ); ?>"><?php _e( 'Cuenta de Twitter', 'epic' ); ?>:</label>
                <input type="text" class="widefat" id="<?php echo $this->get_field_id( 'twitter_account' ); ?>" name="<?php echo $this->get_field_name( 'twitter_account' ); ?>" value="<?php echo esc_attr($twitter_account);?>"/>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id( 'twitter_url' ); ?>"><?php _e( 'Escribir la url de la pagina en Twitter', 'epic' ); ?>:</label>
                <input type="text" class="widefat" id="<?php echo $this->get_field_id( 'twitter_url' ); ?>" name="<?php echo $this->get_field_name( 'twitter_url' ); ?>" value="<?php echo esc_attr($twitter_url);?>"/>
            </p>
            <p>
                ...
                <select class="widefat" id="<?php echo $this->get_field_id( 'twitter_theme' ); ?>" name="<?php echo $this->get_field_name( 'twitter_theme' ); ?>">
                    <option value="light" <?php selected($twitter_theme, 'light'); ?>>Claro</option>
                    <option value="dark" <?php selected($twitter_theme, 'dark'); ?>>Oscuro</option>
                </select>
            </p>
            <p>
                <label for="<?php echo $this->get_field_id( 'twitter_link_color' ); ?>"><?php _e( 'Color para enlaces', 'epic' ); ?>:</label>
                <input type="text" class="widefat" id="<?php echo $this->get_field_id( 'twitter_link_color' ); ?>" name="<?php echo $this->get_field_name( 'twitter_link_color' ); ?>" value="<?php echo esc_attr($twitter_link_color);?>"/>
            </p><?php
        }
    }
}
